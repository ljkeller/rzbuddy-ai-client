...
******************************************/
std::vector<string> load_label_file(string label_file_name)
{
    vector<string> list = {};
    vector<string> empty = {};
...
void draw_bounding_box(void)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        std::cout << "result_str: " << result_str.c_str() << ", ";
        std::cout << "c: " << det[i].c << ", ";
        std::cout << "label: " << label_file_map[det[i].c] << "\n";
    }
    std::cout << std::endl;
    return;
}
...
* Description   : Syncs gpio state and application state
* Arguments     : - (uses globals containing detection state)
******************************************/
static void set_leds()
{
    // For performance reasons, we'll use a static representation of the led
    // instead of reading the value from disk.
    static bool current_led_status(false);
    static std::vector<string> label_file_map = load_label_file(label_list);
    if (label_file_map.empty())
    {
        fprintf(stderr,"[ERROR] Failed to load label file: %s\n", label_list.c_str());
        return;
    ...
    auto dogfinder_it = std::find_if(det.begin(), det.end(), [](const detection& detected_obj){
        return label_file_map[detected_obj.c] == "Dog";
    });

    // target_led_status is a proxy representation of finding a dog in the image
    // Another name for target_led_status could be found_dog.
    const bool target_led_status(dogfinder_it != det.end());
    if (current_led_status != target_led_status)
    {
        std::ofstream file(RED_LED_FILEPATH);
        if (file.is_open())
        {
            file << (target_led_status ? LED_ON_VALUE : LED_OFF_VALUE) << std::endl;
            file.close();
            if (file.fail())
            {
                std::cerr << "something went wrong with led write" << std::endl;
            } else {
                current_led_status = target_led_status;
            }

        } else {
            std::cerr << "Failed to open " << RED_LED_FILEPATH << std::endl;
        }
    }
}
...
int8_t print_result(Image* img)
{
    ...
    img->write_string_rgb(str, DRPAI_IN_WIDTH * RESIZE_SCALE + TEXT_WIDTH_OFFSET, LINE_HEIGHT+LINE_HEIGHT_OFFSET, CHAR_SCALE_SMALL, WHITE_DATA);
...
int8_t R_Main_Process()
{
    ...
    while(1)
    {
        ...
        if (img_obj_ready.load())
        {
            ...
            /*use ai results to determine led status*/
            set_leds();
